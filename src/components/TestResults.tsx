...
export default function TestResults({ result, isRunning, complexity }: TestResultsProps) {
  ...
  if (!result) {
    return (
      <div className="h-full bg-surface-variant flex items-center px-4 text-sm text-muted-foreground">
                ...
                {!r.passed && (
                  <div className="mt-1.5 ml-4 bg-background/60 rounded-md p-2.5 font-mono text-xs space-y-0.5">
                    {r.error ? (
                      ...
                      <>
                        <div className="text-muted-foreground">expected <span className="text-emerald-400">{JSON.stringify(r.expected)}</span></div>
                        <div className="text-muted-foreground">received <span className="text-red-400">{JSON.stringify(r.actual)}</span></div>
                      </>
                    )}
                  </div>
                )}
